...
public class frmUsuarios extends Stage {
    ...
    public frmUsuarios(Stage ventanaPadre) {
        ...
        // Buscador y tabla en un solo bloque central
        VBox centro = new VBox(16);
        centro.getStyleClass().add("admin-centro-container");
        centro.setPadding(new Insets(20, 32, 24, 32));

        // Buscador
        txtBuscar.setPromptText("Buscar usuarios por nombre, usuario o email.");
        txtBuscar.getStyleClass().add("admin-buscador");

        VBox buscadorWrapper = new VBox(txtBuscar);
        buscadorWrapper.getStyleClass().add("admin-buscador-wrapper");
    ...
    // Construye la barra de botones en la parte inferior (solo Modificar, Eliminar y Salir)
    private HBox buildBotones() {
        HBox box = new HBox();
        box.getStyleClass().add("botones-container");

        // Aplicar clases CSS (sin el btnGuardar que ahora está en el formulario flotante)
